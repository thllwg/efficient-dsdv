...
public:
  ...
  /**
    * Set installTime
    * \param installTime the installTime value
    *  should only be used in alternative routing tables
    */
   void
   SetInstallTime (Time installtime)
   {
   ...
   /**
    ...
    GetInstallTime ()
    {
      return (Simulator::Now () - m_installTime);
    }
  /**
   * Set lifetime
...
public:
  ...
  /**
     ...
     */
    void
    GetListOfAllValidRoutes (std::map<Ipv4Address, RoutingTableEntry> & allRoutes);
  /**
   ...
   */
  void
  Purge (std::map<Ipv4Address, RoutingTableEntry> & removedAddresses, std::map<Ipv4Address, RoutingTableEntry> &invalidatedAddresses);
  /**
   * Print routing table
   * \param stream the output stream
   */
